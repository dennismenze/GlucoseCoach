...
test('food comparison uses the highest CGM value only within the first two hours', async ({ page }) => {
  ...
  const rows = [
    ...curve(entries[0].when, {
      baseline: 100, peak: 170, peakOffset: 95, twoHour: 150, latePeak: 240, lateOffset: 175,
    }),
    ...curve(entries[1].when, {
      baseline: 100, peak: 160, peakOffset: 110, twoHour: 145, latePeak: 250, lateOffset: 180,
    }),
  ];

  await page.goto('/');
  await expect(page.locator('#meal-analysis article.card.full p.muted')).toContainText(
    'protokollierten Essensbeginn',
  );
  ...
  await expect(comparison.locator('td')).toHaveText([
    'Hafermilch', '2', '2', '65 mg/dl', '103 min', '48 mg/dl',
  ]);
  await expect(page.locator('#food-comparison-note')).toContainText('ersten 120 Minuten');
...
test('CGM decline proxy cannot precede peak and exposes both timer origins', async ({ page }) => {
  const errors = collectBrowserErrors(page);
  ...
  await expect(intro).toContainText('kein Nachweis eines Insulin-Wirkbeginns');
