...
const localDiary = [
  { id: 'local-meal', occasion: 'Frühstück', food: 'lokal' },
  { id: 'local-sport', occasion: 'Sport', activity: 'Spaziergang' },
];
assert.equal(mode.resolveMealSource(localDiary, clinical, null), mode.SOURCE_LOCAL);
assert.equal(mode.resolveMealSource([], clinical, null), mode.SOURCE_GLOOKO);
assert.equal(
...
assert.equal(analysisDiary.filter((entry) => entry.source === 'glooko').length, 2);
assert.deepEqual(
  mode.buildAnalysisDiary(localDiary, clinical, mode.SOURCE_LOCAL),
  localDiary,
);

assert.equal(mode.MEAL_SOURCE_KEY, 'glucosecoach-meal-source-v1');
assert.equal(mode.GLOOKO_CARBS_ASSOCIATION_MINUTES, 10);
console.log('Glooko meal-source contracts passed');
