...
function mealCurve(when, options) {
  ...
  for (let offset = 5; offset <= 180; offset += 5) {
    ...
    if (offset <= options.peakOffset) {
      value = Math.round(baseline + (options.peak - baseline) * (offset / options.peakOffset));
    } else if (offset <= 120) {
      value = Math.round(
...
const entries = [
  { id: 'oat-1', when: '2026-08-17T07:30', occasion: 'Frühstück', food: 'Hafermilch', illness: 'nein' },
  { id: 'oat-2', when: '2026-08-18T07:30', occasion: 'Frühstück', food: 'Hafermilch', illness: 'nein' },
];
...
console.log('Two-hour peak and sustained-decline contracts passed');
